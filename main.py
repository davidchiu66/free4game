...
def run_automation():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto(target_url, timeout=60000) # 给初始加载更长的时间
            ...
            for attempt in range(1, max_retries + 1):
                ...
                if not is_page_loaded:
                    print("⚠️ 警告：未检测到 Console 标题。可能是白屏、网络缓慢或遇到了 Cloudflare 验证。")
                    
                    # 检查是否遇到了 Cloudflare 等人机验证盾牌
                    if page.get_by_text("Just a moment", exact=False).is_visible() or page.get_by_text("Cloudflare", exact=False).is_visible():
                        print("🛡️ 检测到安全拦截，等待 15 秒让盾牌自动解开...")
                        time.sleep(15)
                    ...
                    time.sleep(5)
                    continue # 【关键修复】直接跳入下一次循环，绝对不去盲目点击！
                ...
                try:
                    ...
                    print(f"⚠️ 点击按钮时出错 (可能被遮挡): {str(click_err).split('Call log:')[0]}")
                    continue
                
                print("正在等待 90 秒 (让后台广告播放)...")
                time.sleep(90) 
